...
write.csv(markers, "results/04_R_analysis/03_annotation/all_cluster_markers.csv", row.names = FALSE)

# Top 10 per cluster
top10 <- markers %>% group_by(cluster) %>% top_n(10, avg_log2FC)
write.csv(top10, "results/04_R_analysis/03_annotation/top10_markers_per_cluster.csv", row.names = FALSE)
...
ggsave("figures/04_R_analysis/03_annotation/marker_dotplot.png", p1, width = 14, height = 10)
...
ggsave("figures/04_R_analysis/03_annotation/umap_annotated.png", p2, width = 12, height = 10)

# Cell type counts
ct_counts <- table(obj$cell_type_integrated, obj$stage)
write.csv(ct_counts, "results/04_R_analysis/03_annotation/celltype_by_stage.csv")

saveRDS(obj, "results/04_R_analysis/03_annotation/purkinje_annotated.rds")
