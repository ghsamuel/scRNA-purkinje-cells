...
ggsave("figures/04_R_analysis/04_purkinje_subtypes/purkinje_subtypes_umap.png", p1, width = 10, height = 8)
...
ggsave("figures/04_R_analysis/04_purkinje_subtypes/foxp_dotplot.png", p2, width = 10, height = 5)

p3 <- FeaturePlot(purkinje, features = foxp_genes, ncol = 3, order = TRUE)
ggsave("figures/04_R_analysis/04_purkinje_subtypes/foxp_featureplots.png", p3, width = 18, height = 6)

# Find subtype markers
markers <- FindAllMarkers(purkinje, only.pos = TRUE, min.pct = 0.25, 
                          logfc.threshold = 0.25, verbose = FALSE)
write.csv(markers, "results/04_R_analysis/04_purkinje_subtypes/purkinje_subtype_markers.csv", 
          row.names = FALSE)

# Distribution across stages
stage_dist <- table(purkinje$seurat_clusters, purkinje$stage)
write.csv(stage_dist, "results/04_R_analysis/04_purkinje_subtypes/subtype_by_stage.csv")

saveRDS(purkinje, "results/04_R_analysis/04_purkinje_subtypes/purkinje_subtypes.rds")
